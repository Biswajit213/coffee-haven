...
const team = [
  { name: 'Marco Rossi', role: 'Head Roaster', bio: '20+ years of coffee expertise from Milan to Seattle.' },
  { name: 'Aiko Tanaka', role: 'Lead Barista', bio: 'World Barista Championship finalist and flavor innovator.' },
  { name: 'Carlos Mendez', role: 'Sourcing Director', bio: 'Travels the globe to find the finest single-origin beans.' },
];
    ...
    <div className="bg-background">
      ...
      <section className="section-padding bg-background">
        <div className="container-max">
          <div className="text-center mb-8 sm:mb-12">
            <h2 className="text-2xl sm:text-3xl md:text-4xl font-heading font-bold text-dark">What Drives Us</h2>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-5 sm:gap-6 md:gap-8">
            {[
              { icon: FiCoffee, title: 'Quality Above All', desc: 'Every bean is hand-selected and tested. We only work with farms that share our obsession with quality.' },
              { icon: FiHeart, title: 'Crafted with Love', desc: 'From the roaster to your cup, every step is done with care and attention to detail.' },
              { icon: FiUsers, title: 'Community First', desc: 'We support fair trade practices and invest in the communities that grow our coffee.' },
            ].map(({ icon: Icon, title, desc }) => (
              <div key={title} className="text-center p-5 sm:p-6 md:p-8 bg-white rounded-xl sm:rounded-2xl shadow-sm">
                ...
                <h3 className="text-base sm:text-lg md:text-xl font-heading font-bold text-dark mb-2 sm:mb-3">{title}</h3>
                <p className="text-gray-500 leading-relaxed text-xs sm:text-sm md:text-base">{desc}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="section-padding bg-primary">
        <div className="container-max text-center">
          ...
          <Link to="/menu" className="bg-accent text-dark font-bold px-8 py-3 sm:px-10 sm:py-4 rounded-lg hover:bg-secondary transition-colors inline-block text-sm sm:text-base">
            Explore Our Coffees
          </Link>
        </div>
      </section>
