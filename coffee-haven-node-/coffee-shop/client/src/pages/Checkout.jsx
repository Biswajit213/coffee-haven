        ...
        <form onSubmit={handleSubmit}>
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-5 sm:gap-8">
            ...
            <div className="lg:col-span-2 space-y-4 sm:space-y-8">
              ...
              <div className="bg-white rounded-xl p-4 sm:p-6 shadow-sm">
                <h2 className="text-base sm:text-xl font-heading font-bold text-dark mb-4 sm:mb-5 flex items-center gap-2">
                  <FiCreditCard className="text-primary" size={18} /> Payment Method
                </h2>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
                  {[
                    { value: 'cash', label: 'Cash on Delivery', icon: '💵', desc: 'Pay when your order arrives' },
                    { value: 'stripe', label: 'Credit/Debit Card', icon: '💳', desc: 'Secure payment with Stripe' },
                  ].map((m) => (
                    <label
                      key={m.value}
                      className={`flex items-center gap-3 sm:gap-4 p-3 sm:p-4 border-2 rounded-xl cursor-pointer transition-colors ${
                        paymentMethod === m.value ? 'border-primary bg-primary/5' : 'border-gray-200 hover:border-gray-300'
                      }`}
                    >
                      <input type="radio" name="payment" value={m.value} checked={paymentMethod === m.value} onChange={() => setPaymentMethod(m.value)} className="text-primary" />
                      <div>
                        <div className="flex items-center gap-1.5">
                          <span>{m.icon}</span>
                          <span className="font-semibold text-dark text-xs sm:text-sm">{m.label}</span>
                        </div>
